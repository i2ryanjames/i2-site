const nav = document.getElementById('mainNav');
  ...
  // ─── Funding Dashboard Bar Animation ───
  (function() {
    var dashboardObserver = new IntersectionObserver(function(entries) {
      entries.forEach(function(entry) {
        if (entry.isIntersecting) {
          var bars = entry.target.querySelectorAll('.funding-card-bar-fill');
          bars.forEach(function(bar) {
            bar.classList.forEach(function(cls) {
              if (cls.indexOf('animated-') === 0) bar.style.width = cls.replace('animated-', '') + '%';
            });
          });
    ...
    var dashboard = document.getElementById('fundingDashboard');
